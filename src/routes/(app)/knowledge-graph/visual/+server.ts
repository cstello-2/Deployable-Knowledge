...
import { getBuiltKnowledgeGraph, searchKnowledgeGraph } from "$lib/server/knowledge-graph";
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const index = await getBuiltKnowledgeGraph(documentIds);
...
function createQueryGraph(
  graph: Awaited<ReturnType<typeof getBuiltKnowledgeGraph>>["graph"],
  query: string,
  results: Array<{
    chunkId: string;
    documentId: string;
