...
import type { ApiProviderApiKeyRequest } from '$lib/types';
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	...
	const body = (await request.json()) as ApiProviderApiKeyRequest;
	...
	const existing = await db.select().from(apiKeys).where(eq(apiKeys.providerId, provider.id)).get();
...
export const DELETE: RequestHandler = async ({ params }) => {
	...
	await db.delete(apiKeys).where(eq(apiKeys.providerId, provider.id));
