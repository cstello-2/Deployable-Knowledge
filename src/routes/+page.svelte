<script lang="ts">
	...
	type PaneWindow = WindowDefinition & { height: number | null; collapsed: boolean };
	...
	function windowsFor(column: WindowColumn): PaneWindow[] {
		return $visibleWindows.filter((window) => window.column === column) as PaneWindow[];
	}
	...
	function paneWindowMinHeight(window: PaneWindow) {
		return window.collapsed ? COLLAPSED_PANE_WINDOW_HEIGHT : MIN_PANE_WINDOW_HEIGHT;
	}

	function findWindowElement(id: string) {
		return (
			Array.from(columnsElement.querySelectorAll<HTMLElement>('[data-window-id]')).find(
				(element) => element.dataset.windowId === id
			) ?? null
		);
	}

	function startPaneResize(column: WindowColumn, index: number, event: PointerEvent) {
		...
		const onMove = (moveEvent: PointerEvent) => {
			...
			const beforeHeight = Math.min(maxBefore, Math.max(minBefore, rawBeforeHeight));
		...
		>
			{#each windowsFor(columns[0].id) as window, index (window.id)}
				...
				{#if index < windowsFor(columns[0].id).length - 1}
					<button
						class="pane-resize"
						class:dragging={activePaneResize === paneResizeId(columns[0].id, index)}
						type="button"
						title="Resize windows"
						aria-label="Resize windows"
						data-window-action
						onpointerdown={(event) => startPaneResize(columns[0].id, index, event)}
					>
		...
		>
			{#each windowsFor(columns[1].id) as window, index (window.id)}
				...
				{#if index < windowsFor(columns[1].id).length - 1}
					<button
						class="pane-resize"
						class:dragging={activePaneResize === paneResizeId(columns[1].id, index)}
						type="button"
						title="Resize windows"
						aria-label="Resize windows"
						data-window-action
						onpointerdown={(event) => startPaneResize(columns[1].id, index, event)}
					>
						<span aria-hidden="true"></span>
					</button>
				{/if}
