...
export type ThemeColor = 'Classic' | 'Purple' | 'Blue' | 'Yellow' | 'Green' | 'High Contrast';
...
export function applyThemeSettings(settings = readThemeSettings()) {
	document.documentElement.setAttribute('data-theme-mode', settings.mode.toLowerCase());
	document.documentElement.setAttribute('data-theme-color', settings.color.toLowerCase());
}

function parseMode(value: string | null): ThemeMode {
	return themeModes.includes(value as ThemeMode) ? (value as ThemeMode) : defaultTheme.mode;
}

function parseColor(value: string | null): ThemeColor {
	return themeColors.includes(value as ThemeColor) ? (value as ThemeColor) : defaultTheme.color;
}
