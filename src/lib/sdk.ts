...
const JSON_POST = { ...JSON_HEADERS, 'Content-Type': 'application/json' } satisfies HeadersInit;
...
async function ok<T extends JsonReadableResponse>(response: T): Promise<T> {
	if (!response.ok) {
		...
		throw new Error(errorMessageFromBody(data) || `${response.status} ${response.statusText}`);
	}
...
export class DKClient {
	...
	async streamChat(
		...
		const response = await ok(
			await this.fetcher(providerChatPath(request.provider_id || '', request.model_id || '', true), {
				method: 'POST',
	...
	async search(q: string, topK = 5) {
		const response = await ok(
			await this.fetcher(`/search?q=${encodeURIComponent(q)}&top_k=${encodeURIComponent(topK)}`, {
				headers: JSON_HEADERS,
				credentials: 'same-origin'
			})
		);
	...
	async listDocuments() {
		const response = await ok(
			await this.fetcher('/documents', { headers: JSON_HEADERS, credentials: 'same-origin' })
		);
				...
				if (!fakeResponse.ok) {
					asJsonSafe<ErrorBody>(fakeResponse)
						.then((data) => {
							reject(
								new Error(
									errorMessageFromBody(data) || `${xhr.status} ${xhr.statusText}`
								)
							);
						})
	...
	async listDirectory(path = '') {
		...
		const url = relPath
			...
			: '/directory';
		const response = await ok(
			await this.fetcher(url, { headers: JSON_HEADERS, credentials: 'same-origin' })
		);
	...
	async getProgress(jobId: string) {
		const response = await this.fetcher(`/progress/${encodeURIComponent(jobId)}`, {
			headers: JSON_HEADERS,
			credentials: 'same-origin'
		});
	...
	async getCorpusTags() {
		const response = await ok(
			await this.fetcher('/corpus/tags', { headers: JSON_HEADERS, credentials: 'same-origin' })
		);
	...
	async listSessions() {
		const response = await ok(
			await this.fetcher('/sessions', { headers: JSON_HEADERS, credentials: 'same-origin' })
		);
	...
	async renameSession(id: string, title: string) {
		...
		return asJsonSafe<StatusResponse & { session_id: string; title: string }>(response);
	}
	...
	async getUser() {
		const response = await ok(
			await this.fetcher('/user', { headers: JSON_HEADERS, credentials: 'same-origin' })
		);
	...
	async clearProviderApiKey(providerId: string) {
		const response = await ok(
			await this.fetcher(`/providers/${encodeURIComponent(providerId)}/api-key`, {
				method: 'DELETE',
				headers: JSON_HEADERS,
				credentials: 'same-origin'
			})
