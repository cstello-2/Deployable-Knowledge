...
// Prefer segmentID for deduplication; fall back to source+page for older docs
...
// Combine BM25 and vector results into a single deduped map.
// Vector results are inserted first so BM25 fields win on conflict (spread order).
function mergeDocuments(bm25Rank: Document[], vectorRank: Document[]): Map<string, Document> {
  ...
  for (const doc of bm25Rank) {
    ...
    docs.set(key, {
      ...docs.get(key), // keep any vector-only fields
      ...doc,           // BM25 fields overwrite on collision
    });
  }
...
// Cross-encoder reranker: scores each candidate against the query as a pair,
// then returns documents sorted by relevance (highest score first).
export async function reRankData(
  ...
  // Build parallel arrays: one query string per candidate so the tokenizer
  // can batch all (query, passage) pairs in a single call.
  ...
  // logits.data is a flat Float32Array; index i maps to candidates[i]
