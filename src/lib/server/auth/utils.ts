...
export async function getUserByName(username: string): Promise<SafeUser | undefined> {
	return await db
		.select({
			id: users.id,
			username: users.username,
...
export async function validateSessionToken(token: string): Promise<UserSession | undefined> {
...
export async function getSession(sessionId: string): Promise<UserSession | undefined> {
	...
	const session = await db.select().from(userSessions).where(eq(userSessions.id, sessionId)).get();
