...
export function columnSplitter(node: HTMLElement, options: ColumnSplitterOptions = {}) {
	...
	let startPoint = 0;
	let leftStartSize = 0;
	let lastReportedWidth: number | null = null;
	...
	function reportLeftWidth(width: number) {
		const roundedWidth = Math.round(width);
		if (roundedWidth === lastReportedWidth) return;

		lastReportedWidth = roundedWidth;
		settings.onLeftWidthChange(roundedWidth);
	}
	...
	function onResize() {
		...
		reportLeftWidth(applyLeftWidth(Math.round(leftSize())));
	}
