...
<WorkspaceWindow
	{collapsed}
	{closable}
	contentLabel="Sanitized diagnostic console"
	{headerActions}
	{height}
	{id}
	{onClose}
...
>
	...
	>
			...
			>
				...
				<div class="border-r px-3 py-2">
					...
					<strong class="font-mono font-medium"
						>{countLabel(snapshot.counts.activeDocuments)}</strong
					>
				</div>
				<div class="border-r px-3 py-2">
		...
		<div class="flex flex-wrap items-center gap-1.5 border-b px-3 py-2">
			...
			{/each}
			<Button class="ml-auto" onclick={downloadReport} size="sm" variant="outline">
				<Download />
				Download .txt
			</Button>
		</div>
		...
		<ScrollArea bind:viewportRef={viewport} class="min-h-0">
			...
			>
				{#if diagnosticsStore.error}
					<p
						class="m-0 rounded border border-destructive/50 bg-destructive/10 px-2 text-destructive"
					>
