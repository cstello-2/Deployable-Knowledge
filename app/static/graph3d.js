...
const KIND_COLORS = { entity: 0x4f9cf0, acronym: 0x3fb27f, measure: 0xe0a23c };
...
function initScene() {
    ...
    for (let i = 0; i < 1500; i++)
        sp.push((Math.random() - 0.5) * 5000, (Math.random() - 0.5) * 5000, (Math.random() - 0.5) * 5000);
...
function buildGraph(data) {
    ...
    [...data.nodes].sort((a, b) => b.degree - a.degree).slice(0, 70).forEach((n) => {
...
function step(dt) {
    ...
    if (keys["KeyD"] || keys["ArrowRight"])
        v.add(right);
    if (keys["Space"] || keys["KeyR"])
        v.add(up);
    if (keys["ShiftLeft"] || keys["KeyF"])
        v.sub(up);
    if (v.lengthSq() > 0)
        camera.position.add(v.normalize().multiplyScalar(SPEED * dt));
...
async function main() {
    ...
    try {
        ...
        el("status").textContent = "could not load /graph/data — is the graph built?";
        return;
    }
    if (!data.nodes || !data.nodes.length) {
        el("status").textContent = "no graph yet — run `make graph` first.";
        return;
    }
    initScene();
    buildGraph(data);
    ...
    window.dk3d = { step, keys, get pos() { return camera.position.toArray().map((x) => Math.round(x)); } };
