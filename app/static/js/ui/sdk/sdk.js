...
export class DKClient {
  async chat({
    ...
    fd.append("rag_enabled", String(rag_enabled));

    // Only send retrieval top_k when RAG is on.
    // This top_k controls document retrieval, not Ollama sampling.
    if (rag_enabled) {
  ...
  async streamChat(req, { onMeta, onDelta, onDone, onError, signal } = {}) {
    ...
    if (req.inactive && req.inactive.length) {
      params.set("inactive", JSON.stringify(req.inactive));
    }

    params.set("template_id", req.template_id || "rag_chat");
    params.set("rag_enabled", String(req.rag_enabled ?? true));

    // Only send retrieval top_k when RAG is on.
    // This prevents the RAG-off request from carrying retrieval settings.
    if (req.rag_enabled ?? true) {
  ...
  async listDocuments() {
    const res = await ok(await fetch("/documents", {
      headers: JSON_HEADERS,
      credentials: "same-origin",
    }));
  ...
  async removeDocument(source) {
    ...
    const res = await ok(await fetch("/remove", {
      method: "POST",
      body: fd,
      credentials: "same-origin",
    }));
  ...
  async uploadDocuments(files) {
    ...
    const res = await ok(await fetch("/upload", {
      method: "POST",
      body: fd,
      credentials: "same-origin",
    }));
  ...
  async listSessions() {
    const res = await ok(await fetch("/sessions", {
      headers: JSON_HEADERS,
      credentials: "same-origin",
    }));
  ...
  async getSegment(id) {
    const res = await ok(await fetch(`/segments/${encodeURIComponent(id)}`, {
      headers: JSON_HEADERS,
      credentials: "same-origin",
    }));
  ...
  async startNewSession() {
    const res = await ok(await fetch("/session", {
      method: "POST",
      headers: JSON_HEADERS,
      credentials: "same-origin",
    }));
  ...
  async getUser() {
    const res = await ok(await fetch("/user", {
      headers: JSON_HEADERS,
      credentials: "same-origin",
    }));
  ...
  async listPromptTemplates() {
    const res = await ok(await fetch("/api/prompt-templates", {
      headers: JSON_HEADERS,
      credentials: "same-origin",
    }));
