...
export function render(config, winId) {
  ...
  const newChat = el("button", { class: "btn", type: "button", id: `${winId}-new-chat` }, ["New Chat"]);
  const send = el("button", { class: "btn", type: "button", id: `${winId}-send` }, ["Send"]);
  const bar = el("div", { class: "chat-input" }, [input, newChat, send]);
  wrap.append(log, bar);
