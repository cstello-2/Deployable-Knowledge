...
function expand(name) {
    ...
    viz.updateWithCypher(`MATCH (n:Entity {name:'${safe}'})-[r:CO_OCCURS]-(m:Entity) RETURN n, r, m LIMIT 40`);
}
...
function search(term) {
    ...
    viz.renderWithCypher(`MATCH (n:Entity) WHERE toLower(n.name) CONTAINS toLower('${safe}') ` +
...
function wireNetwork() {
    ...
    net.off("click");
    net.off("doubleClick");
    net.on("click", (p) => { if (p.nodes && p.nodes.length)
        showDetail(nodeName(p.nodes[0])); });
    net.on("doubleClick", (p) => { if (p.nodes && p.nodes.length)
        expand(nodeName(p.nodes[0])); });
}
async function main() {
    ...
    try {
        ...
        setStatus("could not read /graph/viz-config");
        return;
    }
    initialCypher = cfg.initialCypher;
    const labelCfg = { label: "name", value: "count", group: "kind" };
    ...
    if (NeoVis.NEOVIS_ADVANCED_CONFIG) {
        labelCfg[NeoVis.NEOVIS_ADVANCED_CONFIG] = {
            function: {
                title: (node) => `${node.properties.name} · ${node.properties.kind} · seen ${node.properties.count}×`,
            },
        };
    }
    const config = {
    ...
    window.dkGraph = viz; // debug 🔭 handle — also handy for power users in the console.
    viz.registerOnEvent("completed", () => {
        ...
        setTimeout(() => { try {
            viz.network.fit({ animation: false });
        }
        catch (e) { } }, 150);
    });
    viz.registerOnEvent("error", (e) => setStatus("Neo4j error: " + ((e && e.message) || e) + " — is `docker compose up -d neo4j` running and loaded?"));
    viz.render();
    el("go").addEventListener("click", () => search(el("q").value));
