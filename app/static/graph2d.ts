...
declare const vis: any; // vendored global 🔭 (app/static/js/vendor/vis-network.min.js)

const KIND_COLORS: Record<string, string> = { entity: "#4f9cf0", acronym: "#3fb27f", measure: "#e0a23c" };
const el = (id: string) => document.getElementById(id) as HTMLElement;
const setStatus = (m: string) => { el("status").textContent = m; };

let network: any, nodes: any, edges: any;
const seenNodes = new Set<string>(), seenEdges = new Set<string>();
...
function merge(data: any): void {
  for (const n of data.nodes || []) {
    if (!seenNodes.has(n.id)) { seenNodes.add(n.id); nodes.add(nodeObj(n)); }
    else nodes.update({ id: n.id, value: Math.max(1, n.degree) });
  }
  for (const l of data.links || []) {
    const k = edgeKey(l.source, l.target);
    if (!seenEdges.has(k)) { seenEdges.add(k); edges.add({ from: l.source, to: l.target, value: l.weight }); }
  }
}

async function fetchSub(focus: string | null): Promise<any> {
...
function reset(data: any): void {
  nodes.clear(); edges.clear(); seenNodes.clear(); seenEdges.clear();
  merge(data);
  setTimeout(() => { try { network.fit({ animation: false }); } catch (e) {} }, 120);
}
...
async function main(): Promise<void> {
  ...
  (window as any).dkMap = { network, nodes, edges }; // debug 🔭 handle + console power‑use.
  network.on("click", (p: any) => { if (p.nodes && p.nodes.length) showDetail(p.nodes[0]); });
  network.on("doubleClick", async (p: any) => {
    if (p.nodes && p.nodes.length) { setStatus("expanding " + p.nodes[0] + " …"); merge(await fetchSub(p.nodes[0])); }
  });
  ...
  el("go").addEventListener("click", async () => {
    ...
    if (q) { setStatus("searching “" + q + "” …"); reset(await fetchSub(q)); }
  });
  el("q").addEventListener("keydown", (e: any) => { if (e.key === "Enter") el("go").click(); });
